...
const SimilarReads = () => {
  ...
  const fetchCurriculum = async (slug) => {
    ...
    try {
      ...
      console.log("fetching curriculum sectionsaaaaa:", sections);
      setCurriculum(sections);
    } catch (error) {
      console.log("Error fetching curriculumzzzzz:", error);
    }
  ...
  const lectures = curriculum.flatMap((course) =>
    course.sections.flatMap((section) =>
      section.lectures.map((lecture) => ({
        title: lecture.title,
        slug: lecture?.slug,
        description: lecture.content.substring(0, 400), // Shorten content as description
  ...
  const shuffledLectures = lectures.sort(() => Math.random() - 0.5).slice(0, 5);
