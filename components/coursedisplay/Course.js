...
import CourseDescription from "./CourseDescrition";
...
export default function HomePage({ course, loading }) {
  ...
  const handleDownload = () => {
    ...
    let yPosition = 30; // Initial Y position for sections
    ...
    course.sections.forEach((section) => {
      ...
      doc.text(section.title, 10, yPosition); // Section title
      yPosition += 15; // Add more padding after section title

      // Add space before the lectures
      section.lectures.forEach((lecture) => {
        // Set font size, color, and make it bold for lecture titles
        doc.setFontSize(14);
        doc.setFont("helvetica", "bold"); // Bold font for lecture title
        doc.setTextColor(51, 51, 51); // Set color to dark gray for lecture title

        // Indent lecture title and description
        doc.text(`- ${lecture.title}`, 15, yPosition); // Indented lecture title and description
        yPosition += 10; // Increase space after each lecture to prevent overlap
      });

      // Extra space after each section to keep sections clearly separated
      yPosition += 10; // More space after each section for clarity
    });
